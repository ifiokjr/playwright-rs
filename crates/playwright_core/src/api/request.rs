...
use crate::imp::request::Request as Impl;
use crate::imp::utils::ResponseTiming;
...
/// - [`event: Page.response`] emitted when/if the response status and headers
///   are received for the request.
/// - [`event: Page.requestFinished`] emitted when the response body is
///   downloaded and the request is complete.
///
/// If request fails at some point, then instead of `'requestfinished'` event
/// (and possibly instead of 'response' event), the  [`event:
/// Page.requestFailed`] event is emitted.
///
...
impl Request {
  ...
  /// engine. ResourceType will be one of the following: `document`,
  ...
  /// `Request` object. The two requests are connected by `redirectedFrom()`
  /// and `redirectedTo()` methods. When multiple server redirects has happened,
  /// it is possible to construct the whole redirect chain by repeatedly
  /// calling `redirectedFrom()`.
  ///
  ...
  /// Returns resource timing information for given request. Most of the timing
  /// values become available upon the response, `responseEnd` becomes
  /// available when request finishes. Find more information at [Resource Timing API](https://developer.mozilla.org/en-US/docs/Web/API/PerformanceResourceTiming).
  ///
  /// ```js
