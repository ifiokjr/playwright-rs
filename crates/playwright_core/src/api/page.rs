pub use crate::api::frame::AddScriptTagBuilder;
pub use crate::api::frame::CheckBuilder;
pub use crate::api::frame::ClickBuilder;
pub use crate::api::frame::DblClickBuilder;
pub use crate::api::frame::FillBuilder;
pub use crate::api::frame::GotoBuilder;
pub use crate::api::frame::HoverBuilder;
pub use crate::api::frame::PressBuilder;
pub use crate::api::frame::SelectOptionBuilder;
pub use crate::api::frame::SetContentBuilder;
pub use crate::api::frame::SetInputFilesBuilder;
pub use crate::api::frame::TapBuilder;
pub use crate::api::frame::TypeBuilder;
pub use crate::api::frame::UncheckBuilder;
pub use crate::api::frame::WaitForFunctionBuilder;
pub use crate::api::frame::WaitForSelectorBuilder;
use crate::api::input_device::*;
use crate::api::Accessibility;
use crate::api::BrowserContext;
use crate::api::ConsoleMessage;
pub use crate::api::Download;
use crate::api::ElementHandle;
use crate::api::FileChooser;
use crate::api::Frame;
pub use crate::api::JsHandle;
use crate::api::Keyboard;
pub use crate::api::Request;
use crate::api::Response;
use crate::api::TouchScreen;
use crate::api::Video;
use crate::api::WebSocket;
use crate::api::Worker;
use crate::imp::core::*;
use crate::imp::frame::Frame as FrameImpl;
use crate::imp::page::EmulateMediaArgs;
pub use crate::imp::page::EventType;
use crate::imp::page::Evt;
pub use crate::imp::page::Media;
use crate::imp::page::Page as Impl;
use crate::imp::page::PdfArgs;
use crate::imp::page::ReloadArgs;
use crate::imp::page::ScreenshotArgs;
use crate::imp::prelude::*;
use crate::imp::utils::ColorScheme;
use crate::imp::utils::DocumentLoadState;
use crate::imp::utils::File;
use crate::imp::utils::FloatRect;
use crate::imp::utils::Length;
use crate::imp::utils::PdfMargins;
use crate::imp::utils::ScreenshotType;
use crate::imp::utils::Viewport;
...
/// using any of Node's native [`EventEmitter`](https://nodejs.org/api/events.html#events_class_eventemitter) methods, such as `on`, `once` or
/// `removeListener`.
///
...
impl Page {
  ...
  /// Returns the main resource response. In case of multiple redirects, the
  /// navigation will resolve with the response of the last redirect. If can
  /// not go back, returns `null`.
  ///
  ...
  /// own viewport size. However, [`method: Browser.newContext`] allows to set
  ...
  ///   script is evaluated in the context of the newly
  ///  attached frame.
  ///
  ...
  /// generate a pdf with `screen` media, call [`method: Page.emulateMedia`]
  /// before calling `page.pdf()`:
  ///
  /// > NOTE: By default, `page.pdf()` generates a pdf with modified colors for
  /// printing. Use the [`-webkit-print-color-adjust`](https://developer.mozilla.org/en-US/docs/Web/CSS/-webkit-print-color-adjust) property to
  ...
  /// If `runBeforeUnload` is `false`, does not run any unload handlers and
  /// waits for the page to be closed. If `runBeforeUnload` is `true` the
  /// method will run unload handlers, but will **not** wait for the page to
  /// close.
  ///
  /// By default, `page.close()` **does not** run `beforeunload` handlers.
  ///
  /// > NOTE: if `runBeforeUnload` is passed as true, a `beforeunload` dialog
  /// might be summoned and should be handled manually via [`event:
  /// Page.dialog`] event.
  ...
  /// and/or the `'prefers-colors-scheme'` media feature, using the
  ...
  ///
  /// > NOTE: [`method: Page.setExtraHTTPHeaders`] does not guarantee the order
...
pub enum Event {
  ...
  /// `confirm` or `beforeunload`. Listener **must** either [`method:
  /// Dialog.accept`] or [`method: Dialog.dismiss`] the dialog - otherwise the
  /// page will [freeze](https://developer.mozilla.org/en-US/docs/Web/JavaScript/EventLoop#never_blocking) waiting for the dialog, and
  /// actions like click will never finish.
  ///
  /// > NOTE: When no [`event: Page.dialog`] listeners are present, all dialogs
  /// are automatically dismissed.
  Dialog,
  DomContentLoaded,
  /// Emitted when attachment download started. User can access basic file
  /// operations on downloaded content via the passed `Download` instance.
  ///
  /// > NOTE: Browser context **must** be created with the `acceptDownloads` set
  /// to `true` when user needs access to the downloaded content. If
  /// `acceptDownloads` is not set, download events are emitted, but the actual
  /// download is not performed and user has no access to the downloaded
  /// files.
  Download(Download),
  /// Emitted when a file chooser is supposed to appear, such as after clicking
  /// the  `<input type=file>`. Playwright can respond to it via setting the
  /// input files using [`method: FileChooser.setFiles`] that can be uploaded
  /// after that.
  ///
  /// ```js
  ...
  Load,
  PageError,
  /// Emitted when the page opens a new tab or window. This event is emitted in
  /// addition to the [`event: BrowserContext.page`], but only for popups
  /// relevant to this page.
  ///
  /// The earliest moment that page is available is when it has navigated to the
  /// initial url. For example, when opening a popup with `window.open('http://example.com')`, this event will fire when the network request to <http://example.com> is
  ...
  /// ```
  ///
  /// > NOTE: Use [`method: Page.waitForLoadState`] to wait until the page gets
  /// to a particular state (you should not need it in most cases).
  Popup(Page),
  /// Emitted when a page issues a request. The request object is read-only. In
  /// order to intercept and mutate requests, see [`method: Page.route`] or
  /// [`method: BrowserContext.route`].
  Request(Request),
  /// Emitted when a request fails, for example by timing out.
  ///
  /// > NOTE: HTTP Error responses, such as 404 or 503, are still successful
  /// responses from HTTP standpoint, so request will complete with [`event:
  /// Page.requestFinished`] event and not with [`event: Page.requestFailed`].
  RequestFailed(Request),
  /// Emitted when a request finishes successfully after downloading the
  /// response body. For a successful response, the sequence of events is
  /// `request`, `response` and `requestfinished`.
  RequestFinished(Request),
  /// Emitted when response status and headers are received for a request. For a
  /// successful response, the sequence of events is `request`, `response` and
  /// `requestfinished`.
  Response(Response),
  WebSocket(WebSocket),
  Worker(Worker),
