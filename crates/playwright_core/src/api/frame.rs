...
use crate::imp::core::*;
use crate::imp::frame::AddScriptTagArgs;
use crate::imp::frame::CheckArgs;
use crate::imp::frame::ClickArgs;
use crate::imp::frame::Evt;
use crate::imp::frame::FillArgs;
use crate::imp::frame::Frame as Impl;
pub use crate::imp::frame::FrameNavigatedEvent;
pub use crate::imp::frame::FrameState;
use crate::imp::frame::GotoArgs;
use crate::imp::frame::HoverArgs;
use crate::imp::frame::Opt;
pub use crate::imp::frame::Polling;
use crate::imp::frame::PressArgs;
use crate::imp::frame::SelectOptionArgs;
use crate::imp::frame::SetContentArgs;
use crate::imp::frame::SetInputFilesArgs;
use crate::imp::frame::TapArgs;
use crate::imp::frame::TypeArgs;
use crate::imp::frame::WaitForFunctionArgs;
use crate::imp::frame::WaitForSelectorArgs;
use crate::imp::prelude::*;
use crate::imp::utils::DocumentLoadState;
use crate::imp::utils::File;
use crate::imp::utils::KeyboardModifier;
use crate::imp::utils::MouseButton;
use crate::imp::utils::Position;
use crate::protocol::generated::LifecycleEvent;

/// At every point of time, page exposes its current frame tree via the
/// [`method: Page.mainFrame`] and [`method: Frame.childFrames`] methods.
///
/// `Frame` object's lifecycle is controlled by three events, dispatched on the
/// page object:
/// - [`event: Page.frameAttached`] - fired when the frame gets attached to the
///   page. A Frame can be attached to the page
///  only once.
/// - [`event: Page.frameNavigated`] - fired when the frame commits navigation
///   to a different URL.
/// - [`event: Page.frameDetached`] - fired when the frame gets detached from
///   the page.  A Frame can be detached from the
///  page only once.
///
...
impl Frame {
  ...
  /// retrieved by calling [`method: Response.status`].
  ///
  /// > NOTE: `frame.goto` either throws an error or returns a main resource
  /// response. The only exceptions are navigation to `about:blank` or
  /// navigation to the same URL with a different hash, which would succeed and
  /// return `null`. > NOTE: Headless mode doesn't support navigation to a PDF
  /// document. See the [upstream issue](https://bugs.chromium.org/p/chromium/issues/detail?id=761295).
  pub fn goto_builder<'a>(&self, url: &'a str) -> GotoBuilder<'a, '_> {
    GotoBuilder::new(self.inner.clone(), url)
  }

  /// This method clicks an element matching `selector` by performing the
  /// following steps: 1. Find an element matching `selector`. If there is
  /// none, wait until a matching element is attached to the DOM. 1. Wait for [actionability](https://playwright.dev/docs/actionability/) checks on the matched element, unless `force` option is set. If the
  ///   element is detached during the checks, the whole action is retried.
  /// 1. Scroll the element into view if needed.
  /// 1. Use [`property: Page.mouse`] to click in the center of the element, or
  /// the specified `position`. 1. Wait for initiated navigations to either
  /// succeed or fail, unless `noWaitAfter` option is set.
  ///
  ...
  /// This method double clicks an element matching `selector` by performing the
  /// following steps: 1. Find an element matching `selector`. If there is
  /// none, wait until a matching element is attached to the DOM. 1. Wait for [actionability](https://playwright.dev/docs/actionability/) checks on the matched element, unless `force` option is set. If the
  ///   element is detached during the checks, the whole action is retried.
  /// 1. Scroll the element into view if needed.
  /// 1. Use [`property: Page.mouse`] to double click in the center of the
  /// element, or the specified `position`. 1. Wait for initiated navigations
  /// to either succeed or fail, unless `noWaitAfter` option is set. Note that
  /// if the   first click of the `dblclick()` triggers a navigation event,
  /// this method will throw.
  ///
  /// When all steps combined have not finished during the specified `timeout`,
  /// this method throws a `TimeoutError`. Passing zero timeout disables this.
  ///
  /// > NOTE: `frame.dblclick()` dispatches two `click` events and a single
  /// `dblclick` event.
  pub fn dblclick_builder<'a>(&self, selector: &'a str) -> DblClickBuilder<'a> {
    DblClickBuilder::new(self.inner.clone(), selector)
  }

  /// This method taps an element matching `selector` by performing the
  /// following steps: 1. Find an element matching `selector`. If there is
  /// none, wait until a matching element is attached to the DOM. 1. Wait for [actionability](https://playwright.dev/docs/actionability/) checks on the matched element, unless `force` option is set. If the
  ///   element is detached during the checks, the whole action is retried.
  /// 1. Scroll the element into view if needed.
  /// 1. Use [`property: Page.touchscreen`] to tap the center of the element, or
  /// the specified `position`. 1. Wait for initiated navigations to either
  /// succeed or fail, unless `noWaitAfter` option is set.
  ///
  /// When all steps combined have not finished during the specified `timeout`,
  /// this method throws a `TimeoutError`. Passing zero timeout disables this.
  ///
  /// > NOTE: `frame.tap()` requires that the `hasTouch` option of the browser
  ...
  /// element, fills it and triggers an `input` event after filling. Note that
  /// you can pass an empty string to clear the input field.
  ///
  /// If the target element is not an `<input>`, `<textarea>` or
  /// `[contenteditable]` element, this method throws an error. However, if
  /// the element is inside the `<label>` element that has an associated [control](https://developer.mozilla.org/en-US/docs/Web/API/HTMLLabelElement/control), the control will be filled
  /// instead.
  ///
  /// To send fine-grained keyboard events, use
  /// [Frame::type_builder](Frame::type_builder).
  ...
  /// no element matching `selector`, the method waits until a matching
  ...
  /// frame.
  ///
  /// This is an inverse of [`method: ElementHandle.contentFrame`]. Note that
  ...
  ///
  /// Wait for the `selector` to satisfy `state` option (either appear/disappear
  /// from dom, or become visible/hidden). If at the moment of calling the
  /// method `selector` already satisfies the condition, the method will return
  /// immediately. If the selector doesn't satisfy the condition for the
  ...
  /// character in the text. `frame.type` can be used to send fine-grained
  /// keyboard events. To fill values in form fields, use [`method:
  /// Frame.fill`].
  ///
  ...
  /// This method hovers over an element matching `selector` by performing the
  /// following steps: 1. Find an element matching `selector`. If there is
  /// none, wait until a matching element is attached to the DOM. 1. Wait for [actionability](https://playwright.dev/docs/actionability/) checks on the matched element, unless `force` option is set. If the
  ///   element is detached during the checks, the whole action is retried.
  /// 1. Scroll the element into view if needed.
  /// 1. Use [`property: Page.mouse`] to hover over the center of the element,
  /// or the specified `position`. 1. Wait for initiated navigations to either
  /// succeed or fail, unless `noWaitAfter` option is set.
  ///
  /// When all steps combined have not finished during the specified `timeout`,
  /// this method throws a `TimeoutError`. Passing zero timeout disables this.
  ...
  /// This method checks an element matching `selector` by performing the
  /// following steps: 1. Find an element matching `selector`. If there is
  /// none, wait until a matching element is attached to the DOM.
  /// 1. Ensure that matched element is a checkbox or a radio input. If not,
  /// this method throws. If the element is already   checked, this method
  /// returns immediately. 1. Wait for actionability checks on the matched
  /// element, unless `force` option is set. If the   element is detached
  /// during the checks, the whole action is retried. 1. Scroll the element
  /// into view if needed. 1. Use [`property: Page.mouse`] to click in the
  /// center of the element. 1. Wait for initiated navigations to either
  /// succeed or fail, unless `noWaitAfter` option is set. 1. Ensure that the
  /// element is now checked. If not, this method throws.
  ///
  /// When all steps combined have not finished during the specified `timeout`,
  /// this method throws a `TimeoutError`. Passing zero timeout disables this.
  ...
  /// none, wait until a matching element is attached to the DOM.
  /// 1. Ensure that matched element is a checkbox or a radio input. If not,
  /// this method throws. If the element is already   unchecked, this method
  /// returns immediately. 1. Wait for [actionability](https://playwright.dev/docs/actionability/) checks on the matched element, unless `force` option is set. If the
  ///   element is detached during the checks, the whole action is retried.
  /// 1. Scroll the element into view if needed.
  /// 1. Use [`property: Page.mouse`] to click in the center of the element.
  /// 1. Wait for initiated navigations to either succeed or fail, unless
  /// `noWaitAfter` option is set. 1. Ensure that the element is now
  /// unchecked. If not, this method throws.
  ///
  /// When all steps combined have not finished during the specified `timeout`,
  /// this method throws a `TimeoutError`. Passing zero timeout disables this.
  ...
  pub async fn add_style_tag(&self, content: &str, url: Option<&str>) -> ArcResult<ElementHandle> {
    upgrade(&self.inner)?
  ...
  ///
  /// The only difference between [`method: Frame.evaluate`] and [`method:
  /// Frame.evaluateHandle`] is that [`method: Frame.evaluateHandle`] returns
  /// `JSHandle`.
  ///
  /// If the function, passed to the [`method: Frame.evaluateHandle`], returns a
  /// Promise, then [`method: Frame.evaluateHandle`] would wait for the
  ...
  /// ```
  ///
  /// `JSHandle` instances can be passed as an argument to the [`method:
  /// Frame.evaluateHandle`]: ```js
  /// const aHandle = await frame.evaluateHandle(() => document.body);
  /// const resultHandle = await frame.evaluateHandle(([body, suffix]) =>
  /// body.innerHTML + suffix, [aHandle, 'hello']); console.log(await
  /// resultHandle.jsonValue()); await resultHandle.dispose();
  /// ```
  pub async fn evaluate_js_handle<T>(&self, expression: &str, arg: Option<T>) -> ArcResult<JsHandle>
  where
    T: Serialize,
  {
    upgrade(&self.inner)?
  ...
  ///
  /// If the function passed to the [`method: Frame.evaluate`] returns a
  /// Promise, then [`method: Frame.evaluate`] would wait for the promise to
  /// resolve and return its value.
  ///
  /// If the function passed to the [`method: Frame.evaluate`] returns a
  /// non-Serializable value, then [`method: Frame.evaluate`] returns
  /// `undefined`. Playwright also supports transferring some additional values
  /// that are not serializable by `JSON`: `-0`, `NaN`, `Infinity`,
  /// `-Infinity`.
  ///
  /// ```js
  ...
  /// ```
  ///
  /// `ElementHandle` instances can be passed as an argument to the [`method:
  /// Frame.evaluate`]:
  ///
  /// ```js
  ...
  /// Returns the return value of `expression`.
  ///
  /// The method finds an element matching the specified selector within the
  /// frame and passes it as a first argument to `expression`. If no elements
  /// match the selector, the method throws an error.
  ///
  /// If `expression` returns a Promise, then [`method: Frame.evalOnSelector`]
  ...
  /// frame and passes an array of matched elements as a first argument to
  /// `expression`.
  ///
  /// If `expression` returns a Promise, then [`method:
  /// Frame.evalOnSelectorAll`] would wait for the promise to resolve and
  /// return its value.
  ///
  /// Examples:
  ///
  ...
  /// The snippet below dispatches the `click` event on the element. Regardless
  /// of the visibility state of the element, `click` is dispatched. This is
  /// equivalent to calling [element.click()](https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/click).
  ///
  /// ```js
  ...
  /// `type`, initializes it with `eventInit` properties and dispatches it on
  /// the element. Events are `composed`, `cancelable` and bubble by default.
  ///
  /// Since `eventInit` is event-specific, please refer to the events
  ...
  ///
  /// You can also specify `JSHandle` as the property value if you want live
  ...
  /// all specified options are present in the `<select>` element and selects
  /// these options.
  ///
  /// If the target element is not a `<select>` element, this method throws an
  /// error. However, if the element is inside the `<label>` element that has
  /// an associated [control](https://developer.mozilla.org/en-US/docs/Web/API/HTMLLabelElement/control), the control will be used instead.
  ///
  ...
  ///
  /// Sets the value of the file input to these file paths or files. If some of
  /// the `filePaths` are relative paths, then they are resolved relative to
  /// the the current working directory. For empty array, clears the selected
  /// files.
  ...
  ///
  /// The [`method: Frame.waitForFunction`] can be used to observe viewport size
  /// change:
  ///
  /// ```js
  /// const { firefox } = require('playwright');  // Or 'chromium' or 'webkit'.
