...
use crate::imp::route::ContinueArgs;
use crate::imp::route::FulfillArgs;
use crate::imp::route::Route as Impl;

/// Whenever a network route is set up with [`method: Page.route`] or [`method:
/// BrowserContext.route`], the `Route` object allows to handle the route.
pub struct Route {
  inner: Weak<Impl>,
}
...
impl Route {
  ...
  ///   means that there is no route to the specified
  ///  host or network.
  /// - `'blockedbyclient'` - The client chose to block the request.
  /// - `'blockedbyresponse'` - The request failed because the response was
  ///   delivered along with requirements which are not
  ///  met ('X-Frame-Options' and 'Content-Security-Policy' ancestor checks, for
  /// instance).
  /// - `'connectionaborted'` - A connection timed out as a result of not
...
impl<'a, 'b, 'c> ContinueBuilder<'a, 'b, 'c> {
  ...
  /// If set changes the request HTTP headers. Header values will be converted
  /// to a string.
  pub fn headers<T>(mut self, x: T) -> Self
  where
    T: IntoIterator<Item = (String, String)>,
