use crate::api::Frame;
use crate::imp::core::*;
use crate::imp::element_handle::CheckArgs;
use crate::imp::element_handle::ClickArgs;
use crate::imp::element_handle::ElementHandle as Impl;
use crate::imp::element_handle::FillArgs;
use crate::imp::element_handle::HoverArgs;
use crate::imp::element_handle::Opt;
use crate::imp::element_handle::PressArgs;
use crate::imp::element_handle::ScreenshotArgs;
use crate::imp::element_handle::SelectOptionArgs;
use crate::imp::element_handle::SetInputFilesArgs;
use crate::imp::element_handle::TapArgs;
use crate::imp::element_handle::TypeArgs;
use crate::imp::element_handle::WaitForSelectorArgs;
use crate::imp::prelude::*;
use crate::imp::utils::ElementState;
use crate::imp::utils::File;
use crate::imp::utils::FloatRect;
use crate::imp::utils::KeyboardModifier;
use crate::imp::utils::MouseButton;
use crate::imp::utils::Position;
use crate::imp::utils::ScreenshotType;
use crate::imp::utils::WaitForSelectorState;

/// ElementHandle represents an in-page DOM element. ElementHandles can be
/// created with the [`method: Page.querySelector`] method.
///
/// ```js
...
/// is disposed with [`method: JSHandle.dispose`]. ElementHandles are
/// auto-disposed when their origin frame gets navigated.
///
/// ElementHandle instances can be used as an argument in [`method:
/// Page.evalOnSelector`] and [`method: Page.evaluate`] methods.
#[derive(Debug)]
...
impl ElementHandle {
  ...
  /// `ElementHandle`'s subtree. If no elements match the selector, returns
  /// `null`.
  ...
  /// throws. If the element is already checked,   this method returns
  /// immediately. 1. Wait for actionability checks on the element, unless
  /// `force` option is set. 1. Scroll the element into view if needed.
  /// 1. Use [`property: Page.mouse`] to click in the center of the element.
  /// 1. Wait for initiated navigations to either succeed or fail, unless
  /// `noWaitAfter` option is set. 1. Ensure that the element is now checked.
  ...
  /// This method checks the element by performing the following steps:
  /// 1. Ensure that element is a checkbox or a radio input. If not, this method
  /// throws. If the element is already   unchecked, this method returns
  /// immediately. 1. Wait for [actionability](https://playwright.dev/docs/actionability/) checks on the element, unless `force` option is set.
  /// 1. Scroll the element into view if needed.
  /// 1. Use [`property: Page.mouse`] to click in the center of the element.
  /// 1. Wait for initiated navigations to either succeed or fail, unless
  ...
  /// this method throws a `TimeoutError`. Passing zero timeout disables this.
  ///
  /// > NOTE: `elementHandle.tap()` requires that the `hasTouch` option of the
  ...
  /// event after filling. Note that you can pass an empty string to clear the
  /// input field.
  ///
  ...
  /// [ElementHandle::type_builder](ElementHandle::type_builder)
  ...
  /// Focuses the element, and then uses [`method: Keyboard.down`] and [`method:
  /// Keyboard.up`].
  ///
  ...
  /// This method returns the bounding box of the element, or `null` if the
  /// element is not visible. The bounding box is calculated relative to the
  /// main frame viewport - which is usually the same as the browser window.
  ///
  /// Scrolling affects the returned bonding box, similarly to
  /// [Element.getBoundingClientRect](https://developer.mozilla.org/en-US/docs/Web/API/Element/getBoundingClientRect). That
  /// means `x` and/or `y` may be negative.
  ///
  /// Elements from child frames return the bounding box relative to the main
  /// frame, unlike the [Element.getBoundingClientRect](https://developer.mozilla.org/en-US/docs/Web/API/Element/getBoundingClientRect).
  ///
  /// Assuming the page is static, it is safe to use bounding box coordinates to
  /// perform input. For example, the following snippet should click the
  ...
  ///
  /// This method waits for the actionability checks, then scrolls element into
  /// view before taking a screenshot. If the element is detached from DOM,
