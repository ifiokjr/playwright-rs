use crate::api::browser::Browser;
use crate::api::browser_context::BrowserContext;
use crate::api::playwright::DeviceDescriptor;
use crate::imp::browser_type::BrowserType as Impl;
use crate::imp::browser_type::ConnectOverCdpArgs;
use crate::imp::browser_type::LaunchArgs;
use crate::imp::browser_type::LaunchPersistentContextArgs;
pub use crate::imp::browser_type::RecordHar;
pub use crate::imp::browser_type::RecordVideo;
use crate::imp::core::*;
use crate::imp::prelude::*;
use crate::imp::utils::BrowserChannel;
use crate::imp::utils::ColorScheme;
use crate::imp::utils::Geolocation;
use crate::imp::utils::HttpCredentials;
use crate::imp::utils::ProxySettings;
...
impl BrowserType {
  ...
  /// Returns browser name. For example: `'chromium'`, `'webkit'` or
  /// `'firefox'`. # Errors
  ...
  /// ```
  ///
  /// > **Chromium-only** Playwright can also be used to control the Google
  /// Chrome or Microsoft Edge browsers, but it works best with the version of
  /// Chromium it is bundled with. There is no guarantee it will work with any
  /// other version. Use `executablePath` option with extreme caution.
  /// >
  /// > If Google Chrome (rather than Chromium) is preferred, a
  /// [Chrome Canary](https://www.google.com/chrome/browser/canary.html) or
  /// [Dev Channel](https://www.chromium.org/getting-involved/dev-channel) build is suggested.
  /// >
  /// > Stock browsers like Google Chrome and Microsoft Edge are suitable for
  /// tests that require proprietary media codecs for video playback. See
  ...
  ///
  /// Launches browser that uses persistent storage located at `userDataDir` and
  /// returns the only context. Closing this context will automatically close
  /// the browser. user_data_dir: Path to a User Data Directory, which stores
  /// browser session data like cookies and local storage. More details for [Chromium](https://chromium.googlesource.com/chromium/src/+/master/docs/user_data_dir.md#introduction) and
  /// [Firefox](https://developer.mozilla.org/en-US/docs/Mozilla/Command_Line_Options#User_Profile). Note that Chromium's user
  /// data directory is the **parent** directory of the "Profile Path" seen at
  /// `chrome://version`.
  ...
  /// This methods attaches Playwright to an existing browser instance using the
  /// Chrome DevTools Protocol.
  ///
  /// The default browser context is accessible via [`method:
  /// Browser.contexts`].
  ///
  /// > NOTE: Connecting over the Chrome DevTools Protocol is only supported for
  /// Chromium-based browsers. A CDP websocket endpoint or http url to connect to. For example `http://localhost:9222/` or
  /// `ws://127.0.0.1:9222/devtools/browser/
  /// 387adf4c-243f-4051-a181-46798f4a46f4`.
...
impl<'a, 'b, 'c> Launcher<'a, 'b, 'c> {
  ...
  //#[doc = "If `true`, Playwright does not pass its own configurations args and
  //#[doc only uses the ones from `args`. If an array is\ngiven, then filters out
  //#[doc the given default arguments. Dangerous option; use with care. Defaults
  //#[doc to `false`."]
  ...
  PersistentContextLauncher<'a, 'b, 'c, 'd, 'e, 'f, 'g, 'h, 'i, 'j, 'k>
{
  ...
  //#[doc = "Optional setting to control whether to omit request content from the
  //#[doc HAR. Defaults to `false`."]
  ...
  //#[doc = "Enables video recording for all pages into the specified directory.
  //#[doc If not specified videos are not recorded. Make sure\nto call [`method:
  //#[doc BrowserContext.close`] for videos to be saved."]
  // record_video_dir: Option<path>,
  //#[doc = "Dimensions of the recorded videos. If not specified the size will be
  //#[doc equal to `viewport` scaled down to fit into\n800x800. If `viewport` is
  //#[doc not configured explicitly the video size defaults to 800x450. Actual
  //#[doc picture of each page will\nbe scaled down if necessary to fit the
  //#[doc specified size."]
  // record_video_size: Option<NotImplementedYet>,
  //#[doc = "**DEPRECATED** Use `recordVideo` instead."] video_size:
  //#[doc Option<NotImplementedYet>, = "**DEPRECATED** Use `recordVideo`
  //#[doc instead."] videos_path: Option<path>,
}
