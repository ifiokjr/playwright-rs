...
use crate::imp::browser_type::RecordHar;
use crate::imp::browser_type::RecordVideo;
use crate::imp::core::*;
...
impl Browser {
  ...
  fn register_new_context(&self, c: Weak<BrowserContext>) -> Result<(), Arc<Error>> {
    self.push_context(c);
    // TODO: options
    // let this = get_object!(self.context()?.lock().unwrap(), &self.guid(),
    // Browser)?; let bc = upgrade(&c)?;
    // bc._options = params
    Ok(())
  }
}
