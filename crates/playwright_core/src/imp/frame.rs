...
impl Frame {
  ...
  pub(crate) fn try_new(ctx: &Context, channel: ChannelOwner) -> Result<Self, Error> {
    ...
    let parent_frame = match parent_frame.map(|OnlyGuid { guid }| get_object!(ctx, &guid, Frame)) {
  ...
  pub(crate) async fn inner_text(&self, selector: &str, timeout: Option<f64>) -> ArcResult<String> {
    let args = SelectorTimeout { selector, timeout };
  ...
  pub(crate) async fn inner_html(&self, selector: &str, timeout: Option<f64>) -> ArcResult<String> {
    let args = SelectorTimeout { selector, timeout };
  ...
  pub(crate) async fn query_selector_all(
    ...
    let elements: Vec<OnlyGuid> = serde_json::from_value((*first).clone()).map_err(Error::Serde)?;
    let es = elements
      .into_iter()
      .map(|OnlyGuid { guid }| get_object!(self.context()?.lock().unwrap(), &guid, ElementHandle))
...
mod tests {
  ...
  use crate::imp::browser::*;
  use crate::imp::browser_context::BrowserContext;
  use crate::imp::browser_type::*;
  use crate::imp::page::Page;
