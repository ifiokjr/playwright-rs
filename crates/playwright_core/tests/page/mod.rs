...
use playwright_core::api::page;
use playwright_core::api::BrowserContext;
use playwright_core::api::File;
use playwright_core::api::Geolocation;
use playwright_core::api::Page;
use playwright_core::api::Viewport;
...
async fn focus_should_work(c: &BrowserContext) {
  ...
  page
    .set_content_builder("<div id=d1 tabIndex=0></div>")
    .set_content()
    .await
    .unwrap();
  assert_eq!(
    page
      .eval::<String>("() => document.activeElement.nodeName")
      .await
      .unwrap(),
    "BODY"
  );
  ...
  assert_eq!(
    page
      .eval::<String>("(s) => document.activeElement.id")
      .await
      .unwrap(),
    "d1"
  );
...
async fn reload_should_worker(c: &BrowserContext) {
  let page = new(c).await;
  page
    .evaluate::<i32, i32>("x => window._foo = x", 10)
    .await
    .unwrap();
...
async fn permissions(c: &BrowserContext, page: &Page, port: u16, which: Which) {
  ...
  page
    .goto_builder(&super::url_static(port, "/empty.html"))
    .goto()
    .await
    .unwrap();
  let geo = || {
    async {
      page
        .eval::<(Option<f64>, Option<i32>)>(snippet)
        .await
        .unwrap()
    }
  };
...
async fn accessibility(c: &BrowserContext) {
  let p = new(c).await;
  use playwright_core::api::accessibility::SnapshotResponse;
...
async fn screenshot_should_work(p: &Page) {
  use playwright_core::api::ScreenshotType;
...
async fn emulate_media(p: &Page) {
  use playwright_core::api::page::Media;
  let screen = || {
    async {
...
async fn input(c: &BrowserContext) {
  let p = new(c).await;
    ...
    p.set_content_builder(r#"<input type="text" value="" />"#)
      .set_content()
  );
  assert_eq!(
...
async fn query_selector_and_eval(c: &BrowserContext) {
  ...
  let handle =
    done!(p.evaluate_element_handle::<()>("() => document.querySelector('div.foo > div')", None));
